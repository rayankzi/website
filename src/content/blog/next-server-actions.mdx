...
<Code code={`/** @type {import('next').NextConfig} */
const nextConfig = {
  experimental: {
    serverActions: true,
  },
...
<Code code={`npm i --save-dev prisma
npm i @prisma/client`}>
  <CodeHeader copyButton>bash</CodeHeader>
  <CodeBlock lang="bash" />
</Code>
...
<Code code={`import { PrismaClient } from "@prisma/client";
...
<Code code={`import prisma from "@/lib/prisma";
...
<Code code={`export default async function Edit() {}`}>
...
<Code code={`async function createBook(data: FormData) {
  "use server";
  ...
}`}>
...
<Code code={`<form action={createBook}>
  ...
</form>`}>

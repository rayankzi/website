...
<Code code={`npx create-next-app --example with-tailwindcss with-tailwindcss-app # npm
...
<Code code={`npm i @3rdweb/hooks @3rdweb/sdk ethers uuid react-hot-toast # npm
...
<Code code={`const supportedChainIds = [4];
const connectors = {
  injected: {},
};
...
<Code code={`import { NextComponentType } from "next";
...
<Code code={`const { connectWallet } = useWeb3();`}>
...
<Code code={`<a className="..." onClick={() => connectWallet("injected")}>
  ...
</a>`}>
  <CodeHeader copyButton>jsx</CodeHeader>
  <CodeBlock lang="jsx" />
...
<Code code={`// get the address of user's wallet
...
<Code code={`<a className="..." onClick={() => mint()}>
  Mint a Cool Robot
</a>`}>
  <CodeHeader copyButton>jsx</CodeHeader>
  <CodeBlock lang="jsx" />
...
<Code code={`const { address } = useWeb3();
...
<Code code={`export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse
) {}`}>
...
<Code code={`const { address } = req.body;`}>
...
<Code code={`const wallet = new ethers.Wallet(
...
<Code code={`const sdk = new ThirdwebSDK(wallet);`}>
  <CodeHeader copyButton>ts</CodeHeader>
  <CodeBlock lang="ts" />
</Code>
...
<Code code={`const nftModule = sdk.getNFTModule("<contract address here>");`}>
...
<Code code={`import { v4 as uuidv4 } from "uuid";`}>
  <CodeHeader copyButton>js</CodeHeader>
  <CodeBlock lang="js" />
</Code>
...
<Code code={`import toast, { Toaster } from "react-hot-toast";`}>
  <CodeHeader copyButton>js</CodeHeader>
  <CodeBlock lang="js" />
</Code>
...
<Code code={`<Toaster position="top-center" reverseOrder={false} />`}>
  <CodeHeader copyButton>jsx</CodeHeader>
  <CodeBlock lang="jsx" />
</Code>

...
<Code code={`const [open, setOpen] = useState(false);`}>
...
<Code code={`<Dialog open={open} onClose={() => setOpen(false)}></Dialog>`}>
  <CodeHeader copyButton>jsx</CodeHeader>
  <CodeBlock lang="jsx" />
</Code>
...
<Code code={`<Dialog open={open} onClose={() => setOpen(false)}>
...
<Code code={`<Dialog.Title>Update Sucessful</Dialog.Title>
<Dialog.Description>
...
<Code code={`<span className="inline-block h-screen align-middle" aria-hidden="true">
  &#8203;
</span>`}>
  <CodeHeader copyButton>jsx</CodeHeader>
  <CodeBlock lang="jsx" />
...
<Code code={`<div className="fixed inset-0 flex items-center justify-center">
  <a

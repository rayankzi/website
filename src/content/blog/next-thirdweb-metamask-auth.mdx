...
<Code code={`npx create-next-app next-metamask-auth`}>
...
<Code code={`cd next-metamask-auth`}>
  <CodeHeader copyButton>bash</CodeHeader>
  <CodeBlock lang="bash" />
</Code>
...
<Code code={`npm i @walletconnect/web3-provider moralis react-moralis # npm
...
<Code code={`NEXT_NEXT_PUBLIC_APP_ID=app_id
...
<Code code={`import { MoralisProvider } from "react-moralis";`}>
...
<Code code={`import { useMoralis } from "react-moralis";`}>
  <CodeHeader copyButton>jsx</CodeHeader>
  <CodeBlock lang="jsx" />
</Code>
...
<Code code={`const { authenticate } = useMoralis();`}>
...
<Code code={`<button onClick={() => authenticate()}>Login</button>`}>
...
<Code code={`const { authenticate, isAuthenticated, logout } = useMoralis();`}>
  <CodeHeader copyButton>js</CodeHeader>
  <CodeBlock lang="js" />
</Code>
...
<Code code={`const { authenticate, isAuthenticated, logout, user } = useMoralis();`}>
  <CodeHeader copyButton>js</CodeHeader>
  <CodeBlock lang="js" />
</Code>

...
<Code code={`npx create-next-app nextjs-typescript-demo --ts`}>
...
<Code code={`import { GetServerSideProps, GetStaticPaths, GetStaticProps } from "next";
...
<Code code={`import { NextComponentType, NextPageContext } from "next";
...
<Code code={`import type { NextApiRequest, NextApiResponse } from "next";
...
<Code code={`export const getStaticProps: GetStaticProps = async () => {};`}>
  <CodeHeader copyButton>ts</CodeHeader>
  <CodeBlock lang="ts" />
</Code>
...
<Code code={`export default interface PostType {
  userId: number;
  id: number;
  title: string;
  body: string;
...
<Code code={`const posts: PostType[] = await res.json();`}>
  <CodeHeader copyButton>ts</CodeHeader>
  <CodeBlock lang="ts" />
</Code>

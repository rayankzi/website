...
<Code code={`npm i @3rdweb/hooks @3rdweb/sdk @3rdweb/react ethers # npm
...
<Code code={`import "../styles/globals.css";
...
<Code code={`const { address, connectWallet } = useWeb3();`}>
...
<Code code={`<a onClick={() => connectWallet("injected")}></a>`}>
...
<Code code={`<a className="group relative inline-block focus:outline-none focus:ring">
...
<Code code={`const { provider } = useWeb3();`}>
  <CodeHeader copyButton>MintToken.tsx</CodeHeader>
  <CodeBlock lang="ts" />
</Code>
...
<Code code={`const amount = ethers.utils.parseUnits("5", 18);
...
<Code code={`import { useWeb3 } from "@3rdweb/hooks";
